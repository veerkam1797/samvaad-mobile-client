import {audioToTextConversion} from '@/apis/API';
import {
  RecordingPresets,
  createAudioPlayer,
          ...
          <>
            ...
            <View style={{flexDirection: 'row', marginTop: 16, gap: 16}}>
              <CommonIconButton
                ...
                iconColor="green"
                onPress={async () => {
                  // Send recorded audio to backend, then notify parent with converted text
                  try {
                    if (!recordedUri) {
                      Alert.alert('No recording', 'Please record audio first.');
                      return;
                    }
                    const converted = await audioToTextConversion(recordedUri);
                    if (props.onConverted) props.onConverted(converted);
                    props.hideDialog();
                  } catch (e) {
                    console.error('Conversion failed', e);
                    Alert.alert('Error', 'Failed to convert audio.');
                  }
                }}
                extraStyle={{}}
                contentStyle={{}}
